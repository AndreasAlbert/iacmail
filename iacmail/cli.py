...
from iacmail.util import get_message_hash, read_address_file, read_message_file, read_user_config_file, prompt_password_if_needed
import tqdm

from iacmail.model import SendingAttempt, Base
import typer
...
def get_n_attempts(message_hash: str, address: str):
    with Session() as con:
        return con.query(SendingAttempt).filter_by(message_hash=message_hash, address=address).count()
...
def check_if_already_sent(message_text: str, address: str) -> bool:
    ...
    with Session() as con:
        attempt = con.query(SendingAttempt).filter_by(message_hash=get_message_hash(message_text), address=address, successful=True).one_or_none()
    
    return attempt is not None

def send_message(message: MIMEMultipart, addresses: list[str], user_config: dict) -> dict:
...
def split_addresses_by_sent(addresses:list[str], message:str):
    ...
    sent_addresses = [address for address in addresses if check_if_already_sent(message, address)]
    unsent_addresses = set(addresses) - set(sent_addresses)

    return sent_addresses, unsent_addresses

@app.command()
def send(address_file: Path = typer.Option(default=None), message_file: Path = typer.Option(default=None), user_config_file: Path = typer.Option(default=None), subject: str = typer.Option(default=None)):
    ...
    logger.info(f"Of {len(addresses)} total addresses, {len(sent_addresses)} have already been sent to.")
    ...
    for address in tqdm.tqdm(unsent_addresses, desc="Sending messages"):
        message = build_message(message_text=message_text, address=address, user_config=user_config, subject=subject)
    ...
    if n_failures:
        logger.info(f"Failed to send {n_failures} messages out of {len(unsent_addresses)}.")
    else:
